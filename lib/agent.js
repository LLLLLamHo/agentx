...
Agent.prototype.sendMessage = function ( message ) {
  ...
  this.logger.info( `>>>>>>>>>>>send message to server: %j', ${message}` );
  console.log('>>>>>>>>>>>send message to server: %j', message);
      ...
      if ( that.state === AgentState.WORK ) { // 未连接时忽略执行
        order.run( function ( err, params ) {
          ...
          if ( params.type == 'node_log' ) {
            console.log( '================ 原始数据 ===================' );
            console.log(params.metrics.data);
            params.metrics.data = require('./config-test');
            console.log( '================ 原始数据 ===================' );
          }
